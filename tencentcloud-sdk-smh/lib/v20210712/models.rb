  ...
  module Smh
    module V20210712
      ...
      class CreateLibraryResponse < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :LibraryId, :AccessDomain, :RequestId

        def initialize(libraryid=nil, accessdomain=nil, requestid=nil)
          @LibraryId = libraryid
      ...
      class LibraryExtension < TencentCloud::Common::AbstractModel
        # @param IsFileLibrary: <p>true 为文件类型媒体库，可存储任何类型文件；false 为媒体类型媒体库，仅可存储照片和视频类型文件。默认为 false。在媒体库创建后不能修改。</p>
        # @type IsFileLibrary: Boolean
        # @param IsMultiSpace: <p>true 为多租户空间媒体库，可创建多个租户空间；false 为单租户空间媒体库，不能创建租户空间，仅可使用默认的单一租户空间。默认为 false。在媒体库创建后不能修改。</p>
        # @type IsMultiSpace: Boolean
        # @param CosStorageClass: <p>保存至 COS 对象存储的文件的存储类型，仅支持 STANDARD、STANDARD_IA、INTELLIGENT_TIERING、MAZ_STANDARD、MAZ_STANDARD_IA 和 MAZ_INTELLIGENT_TIERING，默认为 STANDARD，当使用多 AZ 存储桶时将自动使用 MAZ_ 开头的用于多 AZ 的存储类型，否则自动使用非 MAZ_ 开头的用于非多 AZ 的存储类型。当指定智能分层存储 INTELLIGENT_TIERING 或 MAZ_INTELLIGENT_TIERING 时，需要事先为存储桶开启智能分层存储，否则将返回失败。在媒体库创建后不能修改。</p>
        # @type CosStorageClass: String
        # @param UseRecycleBin: <p>是否开启回收站功能。默认为 false。</p>
        # @type UseRecycleBin: Boolean
        # @param AutoRemoveRecycledDays: <p>当开启回收站时，自动删除回收站项目的天数，不能超过 1095（3 年），指定为 0 则不自动删除，默认为 0。当未开启回收站时，该属性为 null。</p>
        # 注意：此字段可能返回 null，表示取不到有效值。
        # @type AutoRemoveRecycledDays: Integer
        # @param EnableSearch: <p>是否启用文件路径搜索功能。默认为 false。</p>
        # @type EnableSearch: Boolean
        # @param DenyOnQuotaLessThanUsage: <p>设置媒体库或租户空间配额且配额小于已使用存储量时，是否拒绝设置请求。默认为 false。</p>
        # @type DenyOnQuotaLessThanUsage: Boolean
        # @param EnableFileHistory: <p>是否开启历史版本。默认为 false。</p>
        # @type EnableFileHistory: Boolean
        # @param FileHistoryCount: <p>当开启历史版本时，指定单个文件保留的历史版本的数量上限，取值范围为 1 到 999。当未开启历史版本时，该属性可能为 null。</p>
        # 注意：此字段可能返回 null，表示取不到有效值。
        # @type FileHistoryCount: Integer
        # @param FileHistoryExpireDay: <p>当开启历史版本时，指定历史版本保留的最长天数，取值范围为 0 到 999，当指定为 0 时代表永久保留。当未开启历史版本时，该属性可能为 null。</p>
        # 注意：此字段可能返回 null，表示取不到有效值。
        # @type FileHistoryExpireDay: Integer
        # @param MaxDirFileNameLength: <p>目录或文件名的最长长度，不能超过 255，默认为 255。修改该参数不会影响存量目录或文件名，即如果将该字段的值改小，已经存在的长度超过目标值的目录或文件名不会发生变化。</p>
        # @type MaxDirFileNameLength: Integer
        # @param IsPublicRead: <p>是否开启公有读，开启后读操作无需使用访问令牌，默认为 false。仅单租户空间媒体库支持该属性，否则该属性为 null。</p>
        # 注意：此字段可能返回 null，表示取不到有效值。
        # @type IsPublicRead: Boolean
        # @param IsMultiAlbum: <p>媒体类型媒体库是否开启多相簿，开启后可创建一级目录（即相簿）且媒体文件只能保存在各相簿中，否则不能创建相簿且媒体文件只能保存在根目录。默认为 false。仅单租户空间媒体类型媒体库支持该属性，否则该属性为 null。在媒体库创建后不能修改。</p>
        # 注意：此字段可能返回 null，表示取不到有效值。
        # @type IsMultiAlbum: Boolean
        # @param AllowPhoto: <p>媒体类型媒体库是否允许上传照片，默认为 true。仅单租户空间媒体类型媒体库支持该属性，否则该属性为 null。</p>
        # 注意：此字段可能返回 null，表示取不到有效值。
        # @type AllowPhoto: Boolean
        # @param AllowPhotoExtName: <p>当媒体类型媒体库允许上传照片时，指定允许的扩展名，默认为空数组，此时将根据文件扩展名对应的 MIME 类型自动判断。仅单租户空间媒体类型媒体库支持该属性，否则该属性为 null。</p>
        # 注意：此字段可能返回 null，表示取不到有效值。
        # @type AllowPhotoExtName: Array
        # @param AllowVideo: <p>媒体类型媒体库是否允许上传视频，默认为 true。仅单租户空间媒体类型媒体库支持该属性，否则该属性为 null。</p>
        # 注意：此字段可能返回 null，表示取不到有效值。
        # @type AllowVideo: Boolean
        # @param AllowVideoExtName: <p>当媒体类型媒体库允许上传视频时，指定允许的扩展名，默认为空数组，此时将根据文件扩展名对应的 MIME 类型自动判断。仅单租户空间媒体类型媒体库支持该属性，否则该属性为 null。</p>
        # 注意：此字段可能返回 null，表示取不到有效值。
        # @type AllowVideoExtName: Array
        # @param AllowFileExtName: <p>指定文件类型媒体库允许的文件扩展名，默认为空数组，此时允许上传所有类型文件。仅单租户空间文件类型媒体库支持该属性，否则该属性为 null。</p>
        # 注意：此字段可能返回 null，表示取不到有效值。
        # @type AllowFileExtName: Array
        # @param RecognizeSensitiveContent: <p>照片上传时是否进行敏感内容鉴定，默认为 false。仅单租户空间媒体库支持该属性，否则该属性为 null。</p>
        # 注意：此字段可能返回 null，表示取不到有效值。
        # @type RecognizeSensitiveContent: Boolean
        # @param EnableQuota: <p>是否开启媒体库配额</p><p>默认值：false</p><p>创建媒体库指定，创建后不允许修改</p>
        # @type EnableQuota: Boolean
        # @param LibraryQuota: <p>媒体库配额，单位：字节</p><p>取值范围：[1, 9223372036854775807]</p><p>创建媒体库时，开启媒体库配额情况下，此参数必填</p>
        # 注意：此字段可能返回 null，表示取不到有效值。
        # @type LibraryQuota: Integer
        ...
        def initialize(isfilelibrary=nil, ismultispace=nil, cosstorageclass=nil, userecyclebin=nil, autoremoverecycleddays=nil, enablesearch=nil, denyonquotalessthanusage=nil, enablefilehistory=nil, filehistorycount=nil, filehistoryexpireday=nil, maxdirfilenamelength=nil, ispublicread=nil, ismultialbum=nil, allowphoto=nil, allowphotoextname=nil, allowvideo=nil, allowvideoextname=nil, allowfileextname=nil, recognizesensitivecontent=nil, enablequota=nil, libraryquota=nil)
        ...
        def deserialize(params)
          ...
          @EnableQuota = params['EnableQuota']
          @LibraryQuota = params['LibraryQuota']
        end
      end
