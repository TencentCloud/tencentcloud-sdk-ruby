  ...
  module Cvm
    module V20170312
      ...
      class CreateLaunchTemplateRequest < TencentCloud::Common::AbstractModel
        # @param LaunchTemplateName: <p>实例启动模板名称。长度为2~128个英文或中文字符。</p>
        ...
        # @type ImageId: String
        # @param LaunchTemplateVersionDescription: <p>实例启动模板版本描述。长度为2~256个英文或中文字符，默认为空字符。</p>
        ...
        # @param DataDisks: <p>实例数据盘配置信息。若不指定该参数，则默认不购买数据盘。支持购买的时候指定21块数据盘，其中最多包含1块LOCAL_BASIC数据盘或者LOCAL_SSD数据盘，最多包含20块CLOUD_BASIC数据盘、CLOUD_PREMIUM数据盘或者CLOUD_SSD数据盘，详情请参考<a href="https://cloud.tencent.com/document/product/362/5145"> 云硬盘使用限制</a>。</p>
        ...
        # @param InstanceCount: <p>购买实例数量。默认取值：1。指定购买实例的数量不能超过用户所能购买的剩余配额数量，具体配额相关限制详见<a href="https://cloud.tencent.com/document/product/213/2664">CVM实例购买限制</a>。</p>
        ...
        # @type ClientToken: String
        # @param HostName: <p>云服务器的主机名。<br><li>点号（.）和短横线（-）不能作为 HostName 的首尾字符，不能连续使用。</li><li>Windows 实例：名字符长度为[2, 15]，允许字母（不限制大小写）、数字和短横线（-）组成，不支持点号（.），不能全是数字。</li><li>其他类型（Linux 等）实例：字符长度为[2, 60]，允许支持多个点号，点之间为一段，每段允许字母（不限制大小写）、数字和短横线（-）组成。</li></p>
        # @type HostName: String
        # @param ActionTimer: <p>定时任务。通过该参数可以为实例指定定时任务，目前仅支持定时销毁。</p>
        # @type ActionTimer: :class:`Tencentcloud::Cvm.v20170312.models.ActionTimer`
        # @param DisasterRecoverGroupIds: <p>置放群组id，仅支持指定一个。该参数可以通过调用 <a href="https://cloud.tencent.com/document/api/213/17810"> DescribeDisasterRecoverGroups </a> 的返回值中的 <code>DisasterRecoverGroupId</code> 字段来获取。</p>
        ...
        # @param DisableApiTermination: <p>实例销毁保护标志，表示是否允许通过api接口删除实例。取值范围：<br><li>TRUE：表示开启实例保护，不允许通过api接口删除实例</li><li>FALSE：表示关闭实例保护，允许通过api接口删除实例<br></li>默认取值：FALSE。</p>
        ...
        # @param LaunchTemplateTagSpecification: <p>标签描述列表。通过指定该参数可以绑定标签到实例启动模板。</p>
      ...
      class DescribeDisasterRecoverGroupQuotaResponse < TencentCloud::Common::AbstractModel
        ...
        deprecate :CvmInSwGroupQuota, :none, 2026, 9
        deprecate :CvmInSwGroupQuota=, :none, 2026, 9
      ...
      class DisassociateInstancesKeyPairsRequest < TencentCloud::Common::AbstractModel
        # @param InstanceIds: <p>一个或多个待操作的实例ID，每次请求批量实例的上限为100。<br>可以通过以下方式获取可用的实例ID：</p><li>通过登录[控制台](https://console.cloud.tencent.com/cvm/index)查询实例ID。</li><li>通过调用接口 [DescribeInstances](https://cloud.tencent.com/document/api/213/15728) ，取返回信息中的 <code>InstanceId</code> 获取实例ID。</li>
        # @type InstanceIds: Array
        # @param KeyIds: <p>密钥对ID列表，列表长度上限为100。可以通过以下方式获取可用的密钥ID：</p><li>通过登录[控制台](https://console.cloud.tencent.com/cvm/sshkey)查询密钥ID。</li><li>通过调用接口 [DescribeKeyPairs](https://cloud.tencent.com/document/api/213/15699) ，取返回信息中的 <code>KeyId</code> 获取密钥对ID。</li>
        # @type KeyIds: Array
        # @param ForceStop: <p>是否强制关机，默认值为 false。常规场景下，建议手动关机后解绑密钥。取值范围：</p><li>true：先执行强制关机，再解绑密钥。</li><li>false：不执行强制关机，仅支持对已关机状态实例进行解绑操作。</li>
        # @type ForceStop: Boolean
      ...
      class DisasterRecoverGroup < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :DisasterRecoverGroupId, :Name, :Type, :CvmQuotaTotal, :CurrentNum, :InstanceIds, :CreateTime, :Affinity, :Strategy, :PartitionCount, :Tags

        def initialize(disasterrecovergroupid=nil, name=nil, type=nil, cvmquotatotal=nil, currentnum=nil, instanceids=nil, createtime=nil, affinity=nil, strategy=nil, partitioncount=nil, tags=nil)
          @DisasterRecoverGroupId = disasterrecovergroupid
          @Name = name
          @Type = type
          @CvmQuotaTotal = cvmquotatotal
      ...
      class EnterRescueModeRequest < TencentCloud::Common::AbstractModel
        ...
        extend Gem::Deprecate
        deprecate :ForceStop, :none, 2026, 9
        deprecate :ForceStop=, :none, 2026, 9
      ...
      class ModifyInstancesAttributeRequest < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :InstanceIds, :InstanceName, :UserData, :SecurityGroups, :CamRoleName, :HostName, :DisableApiTermination, :CamRoleType, :AutoReboot, :EnableJumboFrame

        def initialize(instanceids=nil, instancename=nil, userdata=nil, securitygroups=nil, camrolename=nil, hostname=nil, disableapitermination=nil, camroletype=nil, autoreboot=nil, enablejumboframe=nil)
          @InstanceIds = instanceids
          @InstanceName = instancename
          @UserData = userdata
      ...
      class RebootInstancesRequest < TencentCloud::Common::AbstractModel
        ...
        extend Gem::Deprecate
        deprecate :ForceReboot, :none, 2026, 9
        deprecate :ForceReboot=, :none, 2026, 9
      ...
      class VirtualPrivateCloud < TencentCloud::Common::AbstractModel
        ...
        # @param SubnetId: <p>私有网络子网ID，形如<code>subnet-xxx</code>。有效的私有网络子网ID可通过登录<a href="https://console.cloud.tencent.com/vpc/subnet?rid=1">控制台</a>查询；也可以调用接口  <a href="https://cloud.tencent.com/document/product/215/15784">DescribeSubnets</a> ，从接口返回中的<code>SubnetId</code>字段获取。若在创建子机时SubnetId与VpcId同时传入<code>DEFAULT</code>，则强制使用默认vpc网络。</p>
        # @type SubnetId: String
        # @param AsVpcGateway: <p>是否用作公网网关。公网网关只有在实例拥有公网IP以及处于私有网络下时才能正常使用。取值范围：<li>true：表示用作公网网关</li><li>false：表示不作为公网网关</li>默认取值：false。</p>
        # @type AsVpcGateway: Boolean
        # @param PrivateIpAddresses: <p>私有网络子网 IP 数组，在创建实例、修改实例vpc属性操作中可使用此参数。当前仅批量创建多台实例时支持传入相同子网的多个 IP。</p>
        # @type PrivateIpAddresses: Array
        # @param Ipv6AddressCount: <p>为弹性网卡指定随机生成的 IPv6 地址数量。</p>
