  ...
  module Ess
    module V20201111
      ...
      class ApproverInfo < TencentCloud::Common::AbstractModel
        # @param ApproverType: <p>在指定签署方时，可选择企业B端或个人C端等不同的参与者类型，可选类型如下:</p><p><strong>0</strong>：企业</p><p><strong>1</strong>：个人</p><p><strong>3</strong>：企业“授权签”注：<code>类型为3（企业“授权签”）时，此接口会默认完成该签署方的签署。“授权签”仅进行盖章操作，不能“授权签”名。</code></p><p><strong>7</strong>: 个人“授权签”，适用于个人“授权签”场景。注: <code>个人“授权签”场景为白名单功能，使用前请联系对接的客户经理沟通。</code></p>
        # @type ApproverType: Integer
        # @param ApproverName: <p>签署方经办人的姓名。<br>经办人的姓名将用于身份认证和电子签名，请确保填写的姓名为签署方的真实姓名，而非昵称等代名。</p>
        # @type ApproverName: String
        # @param ApproverMobile: <p>签署方经办人手机号码， 支持中国大陆手机号11位数字(无需加+86前缀或其他字符)。<br>请确认手机号所有方为此合同签署方。</p>
        ...
        # @param NotifyType: <p>通知签署方经办人的方式,  有以下途径:</p><ul><li>  **SMS**  :  (默认)短信</li><li>  **EMAIL**  :  邮箱</li><li>  **ALL**  :  短信+邮箱</li><li>   **NONE**   : 不通知</li></ul><p>注意：<br><code>如果使用的是通过文件发起合同（CreateFlowByFiles），NotifyType必须 是 sms 才会发送短信</code></p><p>枚举值：</p><ul><li>SMS： 短信通知</li><li>EMAIL： 邮件通知</li><li>ALL： 短信+邮件通知</li><li>NONE： 不做任何形式的通知</li></ul>
        # @type NotifyType: String
        # @param ApproverRole: <p>收据场景设置签署人角色类型, 可以设置如下<b>类型</b>:</p><ul><li> **1**  :收款人</li><li>   **2**   :开具人</li><li>   **3** :见证人</li></ul>注: <code>收据场景为白名单功能，使用前请联系对接的客户经理沟通。</code>
        ...
        # @param VerifyChannel: <p><font color="red">【已不再使用】</font>签署意愿确认渠道，默认为WEIXINAPP:人脸识别</p><p>注: 该字段已不再使用, 请用ApproverSignTypes签署人签署合同时的认证方式代替, 新客户可请用ApproverSignTypes来设置</p>
        ...
        # @param UserId: <p>签署人userId，仅支持本企业的员工userid， 可在控制台组织管理处获得</p><p>注：<br>如果传进来的<font color="red">UserId已经实名， 则忽略ApproverName，ApproverIdCardType，ApproverIdCardNumber，ApproverMobile这四个入参</font>（会用此UserId实名的身份证和登录的手机号覆盖）</p>
        # @type UserId: String
        # @param ApproverSource: <p>在企微场景下使用，需设置参数为<strong>WEWORKAPP</strong>，以表明合同来源于企微。</p>
        # @type ApproverSource: String
        # @param CustomApproverTag: <p>在企业微信场景下，表明该合同流程为或签，其最大长度为64位字符串。<br>所有参与或签的人员均需具备该标识。<br>注意，在合同中，不同的或签参与人必须保证其CustomApproverTag唯一。<br>如果或签签署人为本方企业微信参与人，则需要指定ApproverSource参数为WEWORKAPP。</p>
        # @type CustomApproverTag: String
        # @param ApproverOption: <p>可以控制签署方在签署合同时能否进行某些操作，例如拒签、转交他人等。<br>详细操作可以参考开发者中心的ApproverOption结构体。</p>
        # @type ApproverOption: :class:`Tencentcloud::Ess.v20201111.models.ApproverOption`
        # @param ApproverVerifyTypes: <p>【在用文件发起合同场景下才有效，模板发起场景下需要在模板中配置】指定个人签署方查看合同的校验方式,可以传值如下:</p><ul><li>  **1**   : （默认）人脸识别,人脸识别后才能合同内容</li><li>  **2**  : 手机号验证, 用户手机号和参与方手机号(ApproverMobile)相同即可查看合同内容（当手写签名方式为OCR_ESIGN时，该校验方式无效，因为这种签名方式依赖实名认证）</li></ul>注: <ul><li>如果合同流程设置ApproverVerifyType查看合同的校验方式,    则忽略此签署人的查看合同的校验方式</li><li>此字段可传多个校验方式</li></ul>
        # @type ApproverVerifyTypes: Array
        # @param ApproverSignTypes: <p>【在用文件发起合同场景下才有效，模板发起场景下需要在模板中配置】您可以指定签署方签署合同的认证校验方式，可传递以下值：</p><ul><li>**1**：人脸认证，需进行人脸识别成功后才能签署合同；</li><li>**2**：签署密码，需输入与用户在腾讯电子签设置的密码一致才能校验成功进行合同签署；</li><li>**3**：运营商三要素，需到运营商处比对手机号实名信息（名字、手机号、证件号）校验一致才能成功进行合同签署。（如果是港澳台客户，建议不要选择这个）</li><li>**5**：设备指纹识别，需要对比手机机主预留的指纹信息，校验一致才能成功进行合同签署。（iOS系统暂不支持该校验方式）</li><li>**6**：设备面容识别，需要对比手机机主预留的人脸信息，校验一致才能成功进行合同签署。（Android系统暂不支持该校验方式）</li></ul><p>默认为：<br>1(人脸认证 ),2(签署密码),3(运营商三要素),5(设备指纹识别),6(设备面容识别)</p><p>注：</p><ol><li>用<font color="red">模板创建合同场景</font>, 签署人的认证方式需要在配置模板的时候指定, <font color="red">在创建合同重新指定无效</font></li><li>运营商三要素认证方式对手机号运营商及前缀有限制,可以参考<a href="https://qian.tencent.com/developers/company/mobile_support">运营商支持列表类</a>得到具体的支持说明</li><li>校验方式不允许只包含<font color="red">设备指纹识别</font>和<font color="red">设备面容识别</font>，至少需要再增加一种其他校验方式。</li><li><font color="red">设备指纹识别</font>和<font color="red">设备面容识别</font>只支持小程序使用，其他端暂不支持。</li></ol>
        # @type ApproverSignTypes: Array
        # @param ApproverNeedSignReview: <p>此签署人（员工或者个人）签署前，是否需要发起方企业审批，取值如下：<ul><li><strong>false</strong>：（默认）不需要审批，直接签署。</li><li><strong>true</strong>：需要走审批流程。当到对应参与人签署时，会阻塞其签署操作，等待企业内部审批完成。</li></ul>企业可以通过CreateFlowSignReview审批接口通知腾讯电子签平台企业内部审批结果<ul><li>如果企业通知腾讯电子签平台审核通过，签署方可继续签署动作。</li><li>如果企业通知腾讯电子签平台审核未通过，平台将继续阻塞签署方的签署动作，直到企业通知平台审核通过。</li></ul>注：<code>此功能可用于与发起方企业内部的审批流程进行关联，支持手动、“授权签”合同</code><img src="https://qcloudimg.tencent-cloud.cn/raw/b14d5188ed0229d1401e74a9a49cab6d.png" alt="image"></p>
        # @type ApproverNeedSignReview: Boolean
        # @param AddSignComponentsLimits: <p>【在用文件发起合同场景下才有效】<br>在调用<a href="https://qian.tencent.com/developers/companyApis/startFlows/CreateFlowByFiles">用PDF文件创建签署流程</a>创建合同时,如果设置了外层参数SignBeanTag=1(允许签署过程中添加签署控件),则可通过此参数明确规定合同所使用的签署控件类型（骑缝章、普通章法人章等）和具体的印章（印章ID或者印章类型）或签名方式。</p><p>注：<a href="https://qian.tencent.com/developers/company/sign_bean_tag">参考文档和使用示例</a></p>
        # @type AddSignComponentsLimits: Array
        # @param SignInstructionContent: <p>签署须知：支持传入富文本，最长字数：500个中文字符</p>
      ...
      class CancelOrganizationFlowsRequest < TencentCloud::Common::AbstractModel
        # @param Operator: <p>关于渠道应用的相关信息，包括渠道应用标识、第三方平台子客企业标识及第三方平台子客企业中的员工标识等内容，您可以参阅开发者中心所提供的 Agent 结构体以获取详细定义。此接口下面信息必填。<ul><li>渠道应用标识:  Agent.AppId</li><li>第三方平台子客企业标识: Agent.ProxyOrganizationOpenId</li><li>第三方平台子客企业中的员工标识: Agent. ProxyOperator.OpenId</li></ul>第三方平台子客企业和员工必须已经经过实名认证</p>
        # @type Operator: :class:`Tencentcloud::Ess.v20201111.models.UserInfo`
        ...
        attr_accessor :Operator, :Agent, :CancelMessage, :CancelMessageFormat

        def initialize(operator=nil, agent=nil, cancelmessage=nil, cancelmessageformat=nil)
          @Operator = operator
          @Agent = agent
          @CancelMessage = cancelmessage
      ...
      class CancelUserAutoSignEnableUrlRequest < TencentCloud::Common::AbstractModel
        ...
        # @param UserInfo: <p>预撤销链接的用户信息，包含姓名、证件类型、证件号码等信息。</p>
        # @type UserInfo: :class:`Tencentcloud::Ess.v20201111.models.UserThreeFactor`
        # @param Agent: <p>代理企业和员工的信息。<br>在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。</p>
        # @type Agent: :class:`Tencentcloud::Ess.v20201111.models.Agent`
      ...
      class Component < TencentCloud::Common::AbstractModel
        ...
        # @param IsFormType: <p><strong>在通过接口拉取控件信息场景下</strong>，为出参参数，此控件是否通过表单域定位方式生成，默认false-不是，<strong>发起合同时候不要填写此字段留空即可</strong></p>
        ...
        # @param ChannelComponentSource: <p><font color="red">【暂未使用】</font>第三方应用集成中子客企业控件来源。</p><ul><li> <b>0</b> :平台指定；</li><li> <b>1</b> :用户自定义</li></ul>
      ...
      class CreateBatchInformationExtractionTaskResponse < TencentCloud::Common::AbstractModel
        # @param TaskIds: <p>合同智能提取的任务ID列表，每个任务ID为32位字符串。<br>建议开发者保存此任务ID，后续查询合同智能提取详情需要此任务ID。</p><p>注意：<code>返回的索引和ResourceIds数组一致</code></p>
        # @type TaskIds: Array
        # @param RequestId: 唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
        # @type RequestId: String
      ...
      class CreateBatchQuickSignUrlRequest < TencentCloud::Common::AbstractModel
        ...
        # @param ExpiredOn: <p>链接的有效时间，单位为秒。取值范围为 30分钟（1800）-90天（7776000）。如果不传，默认有效期为30分钟。 </p><p>注意： </p><ol><li><code>出于安全性考虑，若有效期大于30分钟，同设备24小时内点击链接查看合同需要进行手机验证码校验。此校验将会扣除短信份额，不建议通过设置此值来缓存签署链接，建议在用户需要签署的时候临时生成链接。</code></li><li><code>动态签署方链接暂不支持修改过期时间。</code></li></ol>
        # @type ExpiredOn: Integer

        attr_accessor :FlowApproverInfo, :Agent, :Operator, :FlowIds, :FlowGroupId, :JumpUrl, :SignatureTypes, :ApproverSignTypes, :SignTypeSelector, :FlowBatchUrlInfo, :Intention, :VideoVerifyTimesLimit, :CacheApproverInfo, :CanBatchReject, :PresetApproverInfo, :CanSkipReadFlow, :ExpiredOn
      ...
      class CreateDocumentRequest < TencentCloud::Common::AbstractModel
        ...
        # @param FormFields: <p>电子文档的填写控件的填充内容。具体方式可以参考<a href="https://qian.tencent.com/developers/companyApis/dataTypes/#formfield">FormField</a>结构体的定义。<ul><li>支持“授权签”传递印章，可通过指定“授权签”控件id，指定印章id来完成</li><li>附件控件支持传入图片、文件资源id，并将内容合成到合同文件中。支持的文件类型有doc、docx、xls、xlsx、html、jpg、jpeg、png、bmp、txt、pdf。需要注意如果传入的资源类型都是图片类型，图片资源会放置在合同文件的末尾，如果传入的资源有非图片类型资源，会将资源放置在附件控件所在页面的下一页。</li></ul>注：只有在控制台编辑模板时，<font color="red">归属给发起方</font>的填写控件（如下图）才能在创建文档的时候进行内容填充。<img src="https://qcloudimg.tencent-cloud.cn/raw/a54a76a58c454593d06d8e9883ecc9b3.png" alt="image"></p>
      ...
      class CreateExtendedServiceAuthInfosRequest < TencentCloud::Common::AbstractModel
        ...
        # @type UserIds: Array
        # @param ExtendServiceType: <p>取值<ul><li>OPEN_SERVER_SIGN：企业“授权签”</li><li>BATCH_SIGN：批量签署</li></ul></p>
      ...
      class CreateFlowByFilesRequest < TencentCloud::Common::AbstractModel
        ...
        # @type FlowName: String
        # @param FileIds: <p>本合同流程需包含的PDF文件资源编号列表，通过<a href="https://qian.tencent.com/developers/companyApis/templatesAndFiles/UploadFiles">UploadFiles</a>接口获取PDF文件资源编号。</p><p>注:  <code>目前，此接口仅支持单个文件发起。</code></p>
        # @type FileIds: Array
        ...
        # @type FlowDescription: String
        # @param FlowType: <p>合同流程的类别分类（可自定义名称，如销售合同/入职合同等），最大长度为200个字符，仅限中文、字母、数字和下划线组成。<br>如果用户已经在控制台创建了自定义合同类型，可以将这里的类型名称传入。 如果没有创建，我们会自动给发起方公司创建此自定义合同类型。<br><img src="https://qcloudimg.tencent-cloud.cn/raw/36582cea03ae6a2559894844942b5d5c.png" alt="image"></p>
        # @type FlowType: String
        # @param Components: <p>发起的时候合成到PDF上的填写控件（需要提供填写控件的内容）</p><ul><li> 单行文本控件      </li><li> 多行文本控件      </li><li> 勾选框控件        </li><li> 数字控件          </li><li> 图片控件          </li><li> 水印控件等          </li></ul><p><img src="https://qcloudimg.tencent-cloud.cn/raw/e004195ee4cb98a7f9bc12eb4a0a0b77.png" alt="image"></p>
        # @type Components: Array
        ...
        # @param UserData: <p>调用方自定义的个性化字段(可自定义此名称)，并以base64方式编码，支持的最大数据大小为 20480长度。</p><p>在合同状态变更的回调信息等场景中，该字段的信息将原封不动地透传给贵方。回调的相关说明可参考开发者中心的<a href="https://qian.tencent.com/developers/company/callback_types_v2">回调通知</a>模块。</p>
        # @type UserData: String
        ...
        # @param ApproverVerifyType: <p>指定个人签署方查看合同的校验方式<ul><li>   <strong>VerifyCheck</strong>  :（默认）人脸识别,人脸识别后才能查看合同内容 </li><li>   <strong>MobileCheck</strong>  :  手机号验证, 用户手机号和参与方手机号（ApproverMobile）相同即可查看合同内容（当手写签名方式为OCR_ESIGN时，该校验方式无效，因为这种签名方式依赖实名认证）</li></ul></p>
        # @type ApproverVerifyType: String
        # @param SignBeanTag: <p>签署方签署控件（印章/签名等）的生成方式：</p><ul><li> **0**：在合同流程发起时，由发起人指定签署方的签署控件的位置和数量。</li><li> **1**：签署方在签署时自行添加签署控件，可以拖动位置和控制数量。</li></ul><p>注：签署方在签署时自行添加签署控件仅支持电子签小程序或web控制台签署，不支持H5</p>
        ...
        # @param OpenDynamicSignFlow: <p>是否开启动态签署合同：<ul><li> <strong>true</strong>：开启动态签署合同，可在发起时可以不传签署人，在签署过程中追加签署人（必须满足：1，发起方企业开启了模块化计费能力；2，发起方企业在企业应用管理中开启了动态签署人2.0能力）    。</li><li> <strong>false</strong>：不开启动态签署合同。</li></ul></p>
      ...
      class CreateFlowGroupByTemplatesResponse < TencentCloud::Common::AbstractModel
        ...
        # @param FlowIds: <p>合同(流程)组中子合同列表.</p>
        # @type FlowIds: Array
        # @param Approvers: <p>合同组签署人信息。</p>
        # @type Approvers: Array
        # @param WorkflowInstanceId: <p>FlowGroupNeedWorkflow</p>
        # @type WorkflowInstanceId: String
        # @param RequestId: 唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
        # @type RequestId: String
      ...
      class CreateFlowRemindsRequest < TencentCloud::Common::AbstractModel
        ...
        # @type RemindTypes: Array
        # @param RemindEmailInfos: <p>邮箱覆盖列表，为指定签署人使用特定邮箱发送邮件催办</p>
        # @type RemindEmailInfos: Array

        attr_accessor :Operator, :FlowIds, :Agent, :RemindTypes, :RemindEmailInfos

        def initialize(operator=nil, flowids=nil, agent=nil, remindtypes=nil, remindemailinfos=nil)
          @Operator = operator
          @FlowIds = flowids
          @Agent = agent
          @RemindTypes = remindtypes
          @RemindEmailInfos = remindemailinfos
        end
        ...
        def deserialize(params)
          ...
          @RemindTypes = params['RemindTypes']
          unless params['RemindEmailInfos'].nil?
            @RemindEmailInfos = []
            params['RemindEmailInfos'].each do |i|
              remindemailinfo_tmp = RemindEmailInfo.new
              remindemailinfo_tmp.deserialize(i)
              @RemindEmailInfos << remindemailinfo_tmp
            end
          end
        end
      end
      ...
      class CreateFlowSignUrlRequest < TencentCloud::Common::AbstractModel
        ...
        # @param ExpiredOn: <p>链接的有效时间，单位为秒。取值范围为 30分钟（1800）-90天（7776000）。如果不传，默认有效期为30分钟。</p><p>注意：</p><ol><li><code>出于安全性考虑，若有效期大于30分钟，同设备24小时内点击链接查看合同需要进行手机验证码校验。此校验将会扣除短信份额，不建议通过设置此值来缓存签署链接，建议在用户需要签署的时候临时生成链接。</code></li><li><code>若生成合同发起方预览链接，有效期大于30分钟，则需要进行验证码校验的手机号为发起方账号绑定的手机号。</code></li></ol>
        # @type ExpiredOn: Integer
      ...
      class CreateIntegrationRoleRequest < TencentCloud::Common::AbstractModel
        ...
        # @param IsGroupRole: <p>角色类型，0:saas角色，1:集团角色<br>默认0，saas角色</p>
      ...
      class CreateMultiFlowSignQRCodeResponse < TencentCloud::Common::AbstractModel
        ...
        # @param SignUrls: <p>一码多签签署码的链接信息，适用于客户系统整合二维码功能。通过链接，用户可直接访问电子签名小程序并签署合同。</p>
      ...
      class CreateOrganizationAuthUrlRequest < TencentCloud::Common::AbstractModel
        ...
        # @param AdminMobileSame: <p>对方打开链接认证时，认证人手机号是否要与接口传递上来的保持一致。</p><ul><li><b>false（默认值）</b>：关闭状态，实际认证时允许与接口传递的信息存在不一致。</li><li><b>true</b>：启用状态，实际认证时必须与接口传递的信息完全相符。</li></ul><p>p.s. 仅在认证人手机号不为空时有效</p>
      ...
      class CreateOrganizationBatchSignUrlResponse < TencentCloud::Common::AbstractModel
        ...
        # @type SignUrl: String
        # @param ExpiredTime: <p>链接过期截止时间，格式为Unix标准时间戳（秒），默认为7天后截止。</p>
        # @type ExpiredTime: Integer
        # @param RequestId: 唯一请求 ID，由服务端生成，每次请求都会返回（若请求因其他原因未能抵达服务端，则该次请求不会获得 RequestId）。定位问题时需要提供该次请求的 RequestId。
        # @type RequestId: String
      ...
      class CreateOrganizationInfoChangeUrlRequest < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :Operator, :ChangeType, :Agent, :UnifiedSocialCreditCode

        def initialize(operator=nil, changetype=nil, agent=nil, unifiedsocialcreditcode=nil)
          @Operator = operator
      ...
      class CreatePartnerAutoSignAuthUrlRequest < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :Agent, :Operator, :AuthorizedOrganizationId, :AuthorizedOrganizationName, :SealTypes, :AuthToMe, :LimitAuthType

        def initialize(agent=nil, operator=nil, authorizedorganizationid=nil, authorizedorganizationname=nil, sealtypes=nil, authtome=nil, limitauthtype=nil)
          @Agent = agent
          @Operator = operator
          @AuthorizedOrganizationId = authorizedorganizationid
      ...
      class CreatePersonAuthCertificateImageRequest < TencentCloud::Common::AbstractModel
        # @param Operator: <p>执行本接口操作的员工信息。<br>注: <code>在调用此接口时，请确保指定的员工已获得所需的接口调用权限，并具备接口传入的相应资源的数据权限。</code></p>
        # @type Operator: :class:`Tencentcloud::Ess.v20201111.models.UserInfo`
        # @param UserName: <p>个人用户名称</p>
        # @type UserName: String
        # @param IdCardType: <p>证件类型，支持以下类型</p><ul><li> ID_CARD  : 中国大陆居民身份证 (默认值)</li><li> HONGKONG_AND_MACAO  : 中国港澳居民来往内地通行证</li><li> HONGKONG_MACAO_AND_TAIWAN  : 中国港澳台居民居住证(格式同中国大陆居民身份证)</li></ul>
        # @type IdCardType: String
      ...
      class CreateSealRequest < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :Operator, :SealName, :Agent, :GenerateSource, :SealType, :SubSealType, :FileName, :Image, :Width, :Height, :Color, :SealHorizontalText, :SealChordText, :SealCentralType, :FileToken, :SealStyle, :SealSize, :TaxIdentifyCode, :SealDescription, :Options

        def initialize(operator=nil, sealname=nil, agent=nil, generatesource=nil, sealtype=nil, subsealtype=nil, filename=nil, image=nil, width=nil, height=nil, color=nil, sealhorizontaltext=nil, sealchordtext=nil, sealcentraltype=nil, filetoken=nil, sealstyle=nil, sealsize=nil, taxidentifycode=nil, sealdescription=nil, options=nil)
          @Operator = operator
          @SealName = sealname
          @Agent = agent
      ...
      class CreateUserAutoSignEnableUrlRequest < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :Operator, :SceneKey, :AutoSignConfig, :UrlType, :NotifyType, :NotifyAddress, :ExpiredTime, :Agent, :UserData, :EndPoint

        def initialize(operator=nil, scenekey=nil, autosignconfig=nil, urltype=nil, notifytype=nil, notifyaddress=nil, expiredtime=nil, agent=nil, userdata=nil, endpoint=nil)
          @Operator = operator
          @SceneKey = scenekey
      ...
      class CreateUserAutoSignSealUrlRequest < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :Operator, :SceneKey, :UserInfo, :Agent, :ExpiredTime, :EndPoint

        def initialize(operator=nil, scenekey=nil, userinfo=nil, agent=nil, expiredtime=nil, endpoint=nil)
          @Operator = operator
          @SceneKey = scenekey
          @UserInfo = userinfo
          @Agent = agent
      ...
      class DescribeCancelFlowsTaskRequest < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :Operator, :TaskId, :Agent, :CancelType

        def initialize(operator=nil, taskid=nil, agent=nil, canceltype=nil)
          @Operator = operator
          @TaskId = taskid
          @Agent = agent
      ...
      class DescribeContractComparisonTaskRequest < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :Operator, :TaskId, :ShowDetail, :Filters

        def initialize(operator=nil, taskid=nil, showdetail=nil, filters=nil)
          @Operator = operator
          @TaskId = taskid
      ...
      class DescribeDraftContractByPromptsTaskResponse < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :Status, :Message, :ContractName, :ResourceId, :ContractUrl, :RequestId

        def initialize(status=nil, message=nil, contractname=nil, resourceid=nil, contracturl=nil, requestid=nil)
          @Status = status
          @Message = message
      ...
      class DescribeExtendedServiceAuthInfosRequest < TencentCloud::Common::AbstractModel
        # @param Operator: <p>执行本接口操作的员工信息。<br>注: <code>在调用此接口时，请确保指定的员工已获得所需的接口调用权限，并具备接口传入的相应资源的数据权限。</code></p>
        # @type Operator: :class:`Tencentcloud::Ess.v20201111.models.UserInfo`
        # @param ExtendServiceType: <p>要查询的扩展服务类型。默认为空，即查询当前支持的所有扩展服务信息。若需查询单个扩展服务的开通情况，请传递相应的值，如下所示：<ul><li>OPEN_SERVER_SIGN：企业“授权签”</li><li>AUTO_SIGN_CAN_FILL_IN：本企业“授权签”合同支持签前内容补充</li><li>BATCH_SIGN：批量签署</li><li>OVERSEA_SIGN：企业与港澳台居民签署合同</li><li>AGE_LIMIT_EXPANSION：拓宽签署方年龄限制</li><li>MOBILE_CHECK_APPROVER：个人签署方仅校验手机号</li><li>HIDE_OPERATOR_DISPLAY：隐藏合同经办人姓名</li><li>ORGANIZATION_OCR_FALLBACK：正楷临摹签名失败后更换其他签名类型</li><li>ORGANIZATION_FLOW_NOTIFY_TYPE：短信通知签署方</li><li>ORGANIZATION_FLOW_EMAIL_NOTIFY：邮件通知签署方</li><li>FLOW_APPROVAL：合同审批强制开启</li><li>ORGANIZATION_FLOW_PASSWD_NOTIFY：签署密码开通引导</li><li>HIDE_ONE_KEY_SIGN：个人签署方手写签名时需逐个手写</li><li>SIGN_SIGNATURE_DEFAULT_SET_HANDWRITE：个人签署方手动签名</li><li>APP_LOGIN：限制企业员工小程序端登录</li><li>PC_LOGIN：限制企业员工网页端登录</li></ul></p>
        # @type ExtendServiceType: String
        # @param Agent: <p>代理企业和员工的信息。<br>在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。</p>
        # @type Agent: :class:`Tencentcloud::Ess.v20201111.models.Agent`
      ...
      class DescribeExtendedServiceAuthInfosResponse < TencentCloud::Common::AbstractModel
        # @param AuthInfoList: <p>服务开通和授权的信息列表，根据查询类型返回所有支持的扩展服务开通和授权状况，或者返回特定扩展服务的开通和授权状况。</p>
      ...
      class DescribeFlowTemplatesRequest < TencentCloud::Common::AbstractModel
        ...
        # @type ShowPreviewComponents: Boolean
        ...
        def initialize(operator=nil, agent=nil, contenttype=nil, filters=nil, offset=nil, limit=nil, applicationid=nil, ischannel=nil, organization=nil, generatesource=nil, withpreviewurl=nil, showpreviewcomponents=nil)
          ...
          @ShowPreviewComponents = showpreviewcomponents
        end
        ...
        def deserialize(params)
          ...
          @ShowPreviewComponents = params['ShowPreviewComponents']
        end
      end
      ...
      class DescribePersonCertificateRequest < TencentCloud::Common::AbstractModel
        # @param Operator: <p>执行本接口操作的员工信息。<br>注: <code>在调用此接口时，请确保指定的员工已获得所需的接口调用权限，并具备接口传入的相应资源的数据权限。</code></p>
        # @type Operator: :class:`Tencentcloud::Ess.v20201111.models.UserInfo`
        # @param UserInfo: <p>个人用户的三要素信息：</p><ul><li>姓名</li><li>证件号</li><li>证件类型</li></ul>
        # @type UserInfo: :class:`Tencentcloud::Ess.v20201111.models.UserThreeFactor`
        # @param Agent: <p>代理企业和员工的信息。<br>在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。</p>
        # @type Agent: :class:`Tencentcloud::Ess.v20201111.models.Agent`
        # @param SceneKey: <p>证书使用场景，可以选择的场景值如下:</p><ul><li> **E_PRESCRIPTION_AUTO_SIGN** :  电子处方场景</li><li> **OTHER** :  通用场景</li></ul>
        # @type SceneKey: String
      ...
      class DisableUserAutoSignRequest < TencentCloud::Common::AbstractModel
        ...
        # @param SceneKey: <p>“授权签”使用的场景值, 可以选择的场景值如下:<ul><li> <strong>E_PRESCRIPTION_AUTO_SIGN</strong> :  电子处方场景</li><li> <strong>OTHER</strong> :  通用场景</li></ul></p>
        # @type SceneKey: String
        # @param UserInfo: <p>需要关闭“授权签”的个人的信息，如姓名，证件信息等。</p>
        # @type UserInfo: :class:`Tencentcloud::Ess.v20201111.models.UserThreeFactor`
        # @param Agent: <p>代理企业和员工的信息。<br>在集团企业代理子企业操作的场景中，需设置此参数。在此情境下，ProxyOrganizationId（子企业的组织ID）为必填项。</p>
        # @type Agent: :class:`Tencentcloud::Ess.v20201111.models.Agent`
      ...
      class FlowApproverDetail < TencentCloud::Common::AbstractModel
        ...
        def deserialize(params)
          ...
          @ApproverRoleName = params['ApproverRoleName']
          @RecipientId = params['RecipientId']
          unless params['ForwardRecords'].nil?
            @ForwardRecords = []
            params['ForwardRecords'].each do |i|
              forwardrecord_tmp = ForwardRecord.new
              forwardrecord_tmp.deserialize(i)
              @ForwardRecords << forwardrecord_tmp
            end
          end
        end
      end
      ...
      class FlowCreateApprover < TencentCloud::Common::AbstractModel
        # @param ApproverType: <p>在指定签署方时，可以选择企业B端或个人C端等不同的参与者类型，可选类型如下：<ul><li> <b>0</b> :企业B端。</li><li> <b>1</b> :个人C端。</li><li> <b>3</b> :企业B端静默（自动）签署，无需签署人参与，“授权签”可以参考<a href="https://qian.tencent.com/developers/company/autosign_guide" target="_blank" rel="noopener noreferrer">“授权签”使用说明</a>文档。</li><li> <b>7</b> :个人C端“授权签”，适用于个人“授权签”场景。注: <b>个人“授权签”场景为白名单功能，使用前请联系对接的客户经理沟通。</b> </li></ul></p>
        # @type ApproverType: Integer
        # @param OrganizationName: <p>组织机构名称。请确认该名称与企业营业执照中注册的名称一致。如果名称中包含英文括号()，请使用中文括号（）代替。注: <code>当approverType=0(企业签署方) 或 approverType=3(企业“授权签”)时，必须指定</code></p>
        # @type OrganizationName: String
        # @param ApproverName: <p>签署方经办人的姓名。<br>经办人的姓名将用于身份认证和电子签名，请确保填写的姓名为签署方的真实姓名，而非昵称等代名。</p><p>在未指定签署人电子签UserId情况下，为必填参数</p>
        # @type ApproverName: String
        # @param ApproverMobile: <p>签署方经办人手机号码， 支持中国大陆手机号11位数字(无需加+86前缀或其他字符)。 此手机号用于通知和用户的实名认证等环境，请确认手机号所有方为此合同签署方。</p><p>注：<code>在未指定签署人电子签UserId情况下，为必填参数</code></p>
        # @type ApproverMobile: String
        # @param ApproverIdCardType: <p>证件类型，支持以下类型</p><ul><li><b>ID_CARD</b>: 居民身份证 (默认值)</li><li><b>HONGKONG_AND_MACAO</b> : 港澳居民来往内地通行证</li><li><b>HONGKONG_MACAO_AND_TAIWAN</b> : 港澳台居民居住证(格式同居民身份证)</li></ul>
        # @type ApproverIdCardType: String
        # @param ApproverIdCardNumber: <p>证件号码，应符合以下规则</p><ul><li>中国大陆居民身份证号码应为18位字符串，由数字和大写字母X组成（如存在X，请大写）。</li><li>中国港澳居民来往内地通行证号码共11位。第1位为字母，“H”字头签发给中国香港居民，“M”字头签发给中国澳门居民；第2位至第11位为数字。</li><li>中国港澳台居民居住证号码编码规则与中国大陆身份证相同，应为18位字符串。</li></ul>
        # @type ApproverIdCardNumber: String
        # @param RecipientId: <p>签署方经办人在模板中配置的参与方ID，与控件绑定，是控件的归属方，ID为32位字符串。</p><p><b>模板发起合同时，该参数为必填项，可以通过<a href="https://qian.tencent.com/developers/companyApis/templatesAndFiles/DescribeFlowTemplates">查询模板信息接口</a>获得。</b><br><b>文件发起合同时，该参数无需传值。</b></p><p>如果开发者后续用合同模板发起合同，建议保存此值，在用合同模板发起合同中需此值绑定对应的签署经办人 。</p>
        # @type RecipientId: String
        # @param VerifyChannel: <p>签署意愿确认渠道，默认为WEIXINAPP:人脸识别</p><p>注: &lt;font color=&quot;red&quot;&gt;不再使用, <code>用ApproverSignTypes签署人签署合同时的认证方式代替, 新客户可请用ApproverSignTypes来设置</code></p>
        # @type VerifyChannel: Array
        # @param NotifyType: <p>通知签署方经办人的方式,  有以下途径:</p><ul><li>  **SMS**  :  (默认)短信</li><li>  **EMAIL**  :  邮件</li><li>  **ALL**  :  邮件+短信</li><li>   **NONE**   : 不通知</li></ul><p>注: <code>既是发起方又是签署方时，不给此签署方发送短信</code></p><p>枚举值：</p><ul><li>SMS： 短信通知</li><li>EMAIL： 邮件通知</li><li>ALL： 邮件通知+短信通知</li><li>NONE： 不做任何形式的通知</li></ul>
        # @type NotifyType: String
        # @param IsFullText: <p>合同强制需要阅读全文，无需传此参数</p>
        # @type IsFullText: Boolean
        # @param PreReadTime: <p>签署方在签署合同之前，需要强制阅读合同的时长，可指定为3秒至300秒之间的任意值。</p><p>若未指定阅读时间，则会按照合同页数大小计算阅读时间，计算规则如下：</p><ul><li>合同页数少于等于2页，阅读时间为3秒；</li><li>合同页数为3到5页，阅读时间为5秒；</li><li>合同页数大于等于6页，阅读时间为10秒。</li></ul>
        # @type PreReadTime: Integer
        # @param UserId: <p>签署人userId，仅支持本企业的员工userid， 可在控制台组织管理处获得</p><p>注：<br>如果传进来的<font color="red">UserId已经实名， 则忽略ApproverName，ApproverIdCardType，ApproverIdCardNumber，ApproverMobile这四个入参</font>（会用此UserId实名的身份证和登录的手机号覆盖）</p>
        # @type UserId: String
        # @param Required: <p><font color="red">字段不再使用</font>，当前只支持true，默认为true</p>
        ...
        # @param RegisterInfo: <p>快速注册相关信息</p>
        # @type RegisterInfo: :class:`Tencentcloud::Ess.v20201111.models.RegisterInfo`
        # @param ApproverOption: <p>签署人个性化能力值，如是否可以转发他人处理、是否可以拒签、是否为动态补充签署人等功能开关。</p>
        # @type ApproverOption: :class:`Tencentcloud::Ess.v20201111.models.ApproverOption`
        # @param JumpUrl: <p>签署完前端跳转的url，暂未使用</p>
        # @type JumpUrl: String
        # @param SignId: <p>签署人的签署ID</p><ul><li>在CreateFlow、CreatePrepareFlow等发起流程时不需要传入此参数，电子签后台系统会自动生成。</li><li>在CreateFlowSignUrl、CreateBatchQuickSignUrl等生成签署链接时，可以通过查询详情接口获取签署人的SignId，然后可以将此值传入，为该签署人创建签署链接。这样可以避免重复传输姓名、手机号、证件号等其他信息。</li></ul>
        # @type SignId: String
        # @param ApproverNeedSignReview: <p>此签署人(员工或者个人)签署时，是否需要发起方企业审批，取值如下：<ul><li><strong>false</strong>：（默认）不需要审批，直接签署。</li><li><strong>true</strong>：需要走审批流程。当到对应参与人签署时，会阻塞其签署操作，等待企业内部审批完成。</li></ul>企业可以通过CreateFlowSignReview审批接口通知腾讯电子签平台企业内部审批结果<ul><li>如果企业通知腾讯电子签平台审核通过，签署方可继续签署动作。</li><li>如果企业通知腾讯电子签平台审核未通过，平台将继续阻塞签署方的签署动作，直到企业通知平台审核通过。</li></ul>注：<code>此功能可用于与发起方企业内部的审批流程进行关联，支持手动、“授权签”合同</code><img src="https://qcloudimg.tencent-cloud.cn/raw/b14d5188ed0229d1401e74a9a49cab6d.png" alt="image"></p>
        # @type ApproverNeedSignReview: Boolean
        # @param SignComponents: <p>签署人签署控件， 此参数仅针对文件发起（CreateFlowByFiles）生效</p><p>合同中的签署控件列表，列表中可支持下列多种签署控件,控件的详细定义参考开发者中心的Component结构体</p><ul><li> 个人签名/印章</li><li> 企业印章</li><li> 骑缝章等签署控件</li></ul><p><code>此参数仅针对文件发起设置生效,模板发起合同签署流程, 请以模板配置为主</code></p>
        # @type SignComponents: Array
        # @param Components: <p>签署人填写控件 此参数仅针对文件发起（CreateFlowByFiles）生效</p><p>合同中的填写控件列表，列表中可支持下列多种填写控件，控件的详细定义参考开发者中心的Component结构体</p><ul><li>单行文本控件</li><li>多行文本控件</li><li>勾选框控件</li><li>数字控件</li><li>图片控件</li><li>动态表格等填写控件</li></ul><p><code>此参数仅针对文件发起设置生效,模板发起合同签署流程, 请以模板配置为主</code></p>
        # @type Components: Array
        ...
        # @param ApproverVerifyTypes: <p>指定个人签署方查看合同的校验方式,可以传值如下:</p><ul><li>  **1**   : （默认）人脸识别,人脸识别后才能合同内容</li><li>  **2**  : 手机号验证, 用户手机号和参与方手机号(ApproverMobile)相同即可查看合同内容（当手写签名方式为OCR_ESIGN时，该校验方式无效，因为这种签名方式依赖实名认证）</li></ul>注: <ul><li>如果合同流程设置ApproverVerifyType查看合同的校验方式,    则忽略此签署人的查看合同的校验方式</li><li>此字段可传多个校验方式</li></ul><p><code>此参数仅针对文件发起设置生效,模板发起合同签署流程, 请以模板配置为主</code></p><p>.</p>
        # @type ApproverVerifyTypes: Array
        # @param ApproverSignTypes: <p>您可以指定签署方签署合同的认证校验方式，可传递以下值：</p><ul><li>**1**：人脸认证，需进行人脸识别成功后才能签署合同；</li><li>**2**：签署密码，需输入与用户在腾讯电子签设置的密码一致才能校验成功进行合同签署；</li><li>**3**：运营商三要素，需到运营商处比对手机号实名信息（名字、手机号、证件号）校验一致才能成功进行合同签署。（如果是港澳台客户，建议不要选择这个）</li><li>**5**：设备指纹识别，需要对比手机机主预留的指纹信息，校验一致才能成功进行合同签署。（iOS系统暂不支持该校验方式）</li><li>**6**：设备面容识别，需要对比手机机主预留的人脸信息，校验一致才能成功进行合同签署。（Android系统暂不支持该校验方式）</li></ul><p>注：</p><ul><li>默认情况下，认证校验方式为人脸认证和签署密码两种形式；</li><li>您可以传递多种值，表示可用多种认证校验方式。</li><li>校验方式不允许只包含设备指纹识别和设备面容识别，至少需要再增加一种其他校验方式。</li><li>设备指纹识别和设备面容识别只支持小程序使用，其他端暂不支持。</li></ul><p>注:<br><code>此参数仅针对文件发起设置生效,模板发起合同签署流程, 请以模板配置为主</code></p>
        # @type ApproverSignTypes: Array
        # @param SignTypeSelector: <p>生成H5签署链接时，您可以指定签署方签署合同的认证校验方式的选择模式，可传递一下值：</p><ul><li>**0**：签署方自行选择，签署方可以从预先指定的认证方式中自由选择；</li><li>**1**：自动按顺序首位推荐，签署方无需选择，系统会优先推荐使用第一种认证方式。</li></ul>注：<code>不指定该值时，默认为签署方自行选择。</code>
        # @type SignTypeSelector: Integer
        # @param Deadline: <p>签署人的签署截止时间，格式为Unix标准时间戳（秒）, 超过此时间未签署的合同变成已过期状态，不能在继续签署</p><p>注: <code>若不设置此参数，则默认使用合同的截止时间，此参数暂不支持合同组子合同</code></p>
        # @type Deadline: Integer
        # @param Intention: <p><b>只有在生成H5签署链接的情形下</b>（ 如调用<a href="https://qian.tencent.com/developers/companyApis/startFlows/CreateFlowSignUrl" target="_blank">获取H5签署链接</a>、<a href="https://qian.tencent.com/developers/companyApis/startFlows/CreateBatchQuickSignUrl" target="_blank">获取H5批量签署链接</a>等接口），该配置才会生效。</p><p>您可以指定H5签署视频核身的意图配置，选择问答模式或点头模式的语音文本。</p><p>注意：</p><ol><li>视频认证为<b>白名单功能，使用前请联系对接的客户经理沟通</b>。</li><li>使用视频认证时，<b>生成H5签署链接必须将签署认证方式指定为人脸</b>（即ApproverSignTypes设置成人脸签署）。</li><li>签署完成后，可以通过<a href="https://qian.tencent.com/developers/companyApis/queryFlows/DescribeSignFaceVideo" target="_blank">查询签署认证人脸视频</a>获取到当时的视频。</li></ol>
        # @type Intention: :class:`Tencentcloud::Ess.v20201111.models.Intention`
        # @param SignEndpoints: <p>进入签署流程的限制，目前支持以下选项：</p><ul><li> <b>空值（默认）</b> :无限制，可在任何场景进入签署流程。</li><li> <b>link</b> :选择此选项后，将无法通过控制台或电子签小程序列表进入填写或签署操作，仅可预览合同。填写或签署流程只能通过短信或发起方提供的专用链接进行。</li></ul>
        # @type SignEndpoints: Array
        # @param NotSaveContact: <p>是否不保存联系人<br>默认 false 保存联系人  true 不保存联系人</p><p>设置这个参数为保存联系人的时候,他方企业签署人会被保存进发起人的联系人中。<br>联系人查看可登录<a href="https://test.qian.tencent.cn/console/">电子签控制台</a> 进行查看。<br>如下图位置：<br><img src="https://qcloudimg.tencent-cloud.cn/raw/fb8a22cd615d24c21acfa0e37e2cd873.png" alt=""></p>
      ...
      class FlowGroupInfo < TencentCloud::Common::AbstractModel
        ...
        # @type Components: Array
        # @param NeedSignReview: <p>发起方企业的签署人进行签署操作是否需要企业内部审批。使用此功能需要发起方企业有参与签署。若设置为true，审核结果需通过接口 <a href="https://qian.tencent.com/developers/companyApis/operateFlows/CreateFlowSignReview">CreateFlowSignReview</a> 通知电子签，审核通过后，发起方企业签署人方可进行签署操作，否则会阻塞其签署操作。注：企业可以通过此功能与企业内部的审批流程进行关联，支持手动、“授权签”合同。示例值：true</p>
        # @type NeedSignReview: Boolean
        # @param AutoSignScene: <p>个人“授权签”场景。发起“授权签”时，需设置对应“授权签”场景，目前仅支持场景：处方单-E_PRESCRIPTION_AUTO_SIGN</p>
        # @type AutoSignScene: String
        # @param FlowDisplayType: <p>在短信通知、填写、签署流程中，若标题、按钮、合同详情等地方存在“合同”字样时，可根据此配置指定文案，可选文案如下：  <ul><li> <b>0</b> :合同（默认值）</li> <li> <b>1</b> :文件</li> <li> <b>2</b> :协议</li><li> <b>3</b> :文书</li></ul>效果如下:<img src="https://qcloudimg.tencent-cloud.cn/raw/e4a2c4d638717cc901d3dbd5137c9bbc.png" alt="FlowDisplayType"></p>
        # @type FlowDisplayType: Integer
      ...
      # 签署人的转交记录详情
      class ForwardRecord < TencentCloud::Common::AbstractModel
        ...
        # @type ForwardMessage: String
        # @param ForwardTime: <p>转交时间</p><p>单位：时间戳（秒级）</p>
        # @type ForwardTime: Integer

        attr_accessor :Name, :Mobile, :ForwardType, :ForwardMessage, :ForwardTime

        def initialize(name=nil, mobile=nil, forwardtype=nil, forwardmessage=nil, forwardtime=nil)
          @Name = name
          @Mobile = mobile
        ...
        def deserialize(params)
          ...
          @ForwardTime = params['ForwardTime']
        end
      end
      ...
      class JumpEvent < TencentCloud::Common::AbstractModel
        # @param JumpEventType: <p>跳转事件枚举</p><p>枚举值：</p><ul><li>1： 企业收录</li><li>2： 超管授权书审核</li><li>3： 企业认证完成</li></ul>
        # @type JumpEventType: Integer
        # @param JumpUrl: <p>为认证成功后页面进行回跳的URL，请确保回跳地址的可用性。<br>Endpoint如果是APP 类型，请传递<font color="red">&quot;true&quot;</font><br>如果 Endpoint 是 H5 类型，请参考文档<a href="https://qian.tencent.com/developers/company/openqianh5/">跳转电子签H5</a></p><p>p.s. 如果Endpoint是 APP，传递的跳转地址无效，不会进行跳转，仅会进行回跳。</p>
        # @type JumpUrl: String
      ...
      class ModifyPartnerAutoSignAuthUrlRequest < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :Agent, :Operator, :AuthorizedOrganizationId, :AuthorizedOrganizationName, :AuthToMe, :SealTypes, :LimitAuthType

        def initialize(agent=nil, operator=nil, authorizedorganizationid=nil, authorizedorganizationname=nil, authtome=nil, sealtypes=nil, limitauthtype=nil)
          @Agent = agent
          @Operator = operator
          @AuthorizedOrganizationId = authorizedorganizationid
          @AuthorizedOrganizationName = authorizedorganizationname
      ...
      class PresetApproverInfo < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :Name, :Mobile, :IdCardNumber, :IdCardType, :OrganizationName

        def initialize(name=nil, mobile=nil, idcardnumber=nil, idcardtype=nil, organizationname=nil)
          @Name = name
          @Mobile = mobile
          @IdCardNumber = idcardnumber
          @IdCardType = idcardtype
      ...
      class RecipientComponentInfo < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :RecipientId, :RecipientFillStatus, :IsPromoter, :Components, :SignComponents

        def initialize(recipientid=nil, recipientfillstatus=nil, ispromoter=nil, components=nil, signcomponents=nil)
          @RecipientId = recipientid
        ...
        def deserialize(params)
          ...
          unless params['SignComponents'].nil?
            ...
            params['SignComponents'].each do |i|
              filledcomponent_tmp = FilledComponent.new
              filledcomponent_tmp.deserialize(i)
              @SignComponents << filledcomponent_tmp
            end
          end
        end
      end
      ...
      # 催办邮件结构体
      class RemindEmailInfo < TencentCloud::Common::AbstractModel
        # @param SignId: <p>签署编号</p>
        # @type SignId: String
        # @param ApproverEmail: <p>指定邮箱地址，催办时使用此邮箱替代 DB 中存储的邮箱</p>
        # @type ApproverEmail: String

        attr_accessor :SignId, :ApproverEmail

        def initialize(signid=nil, approveremail=nil)
          @SignId = signid
          @ApproverEmail = approveremail
        end
      ...
      class TemplateUserFlowType < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :UserFlowTypeId, :Name, :Description, :TemplateNum, :Status

        def initialize(userflowtypeid=nil, name=nil, description=nil, templatenum=nil, status=nil)
          @UserFlowTypeId = userflowtypeid
          @Name = name
          @Description = description
      ...
      class UploadFilesResponse < TencentCloud::Common::AbstractModel
        ...
        attr_accessor :FileIds, :TotalCount, :Deadline, :RequestId

        def initialize(fileids=nil, totalcount=nil, deadline=nil, requestid=nil)
          @FileIds = fileids
          @TotalCount = totalcount
          @Deadline = deadline
          @RequestId = requestid
